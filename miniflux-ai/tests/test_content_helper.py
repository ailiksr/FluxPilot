...
class TestGetContentLength(unittest.TestCase):
    ...
    @patch("core.content_helper.tiktoken", None)
    def test_offline_fallback_counts_cjk_characters(self):
        """The offline fallback must not collapse CJK text to byte-size chunks."""
        entry = {"content": "<p>这是一篇测试文章</p>"}
        self.assertEqual(get_content_length(entry), 8)
